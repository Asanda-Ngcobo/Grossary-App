...
useEffect(() => {
  ...
  return (
    <main>
      <form
        ...
        className="py-2 px-4 rounded-md w-[90%] mt-[5%] ml-[5%] md:w-[40%] md:ml-[25%] grid grid-rows-2 gap-2
         "
      >
        ...
        {/* <input
          type="text"
          name="name"
          className=" text-gray-400 text-2xl font-bold p-3 rounded-md w-full"
          value={`${item.item_name} ${item.item_brand} ${item.item_volume_mass}${item.item_unit}`}
          readOnly
        /> */}

        <label className="text-black text-2xl font-bold">{item.item_name} {item.item_brand} {item.item_volume_mass}{item.item_unit}</label>
   <div className='flex'>
    <h1 className='text-black text-2xl'>R</h1><input
  ref={priceInputRef}
  // type="number"
  step="0.01"
  name="price"
  ...
  className=" text-black text-[100px] font-extrabold p-3 w-full outline-0"
  value={price}
  onChange={(e) => {
...
/>
        ...
        <button
          type="submit"
   className={`${ButtonFont.className} bg-[#1EC677] min-w-[100px]
    h-[40px] rounded-[5px] text-gray-800 font-semibold hover:bg-[#0B2E1E]`}
          disabled={loading}
        >
